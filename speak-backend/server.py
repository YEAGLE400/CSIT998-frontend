"""CSIT998 Speak — on-device, real-time multimodal speaking practice."""
...
SYSTEM_PROMPT = (
    "You are a friendly English speaking coach inside the CSIT998 Speak module. "
    "The user is practicing spoken English through a microphone and may show you "
    "their camera. Keep the conversation natural and encouraging. "
    "You MUST always use the respond_to_user tool to reply. "
    "First transcribe exactly what the user said, then write your conversational "
    "response. When helpful, gently suggest a better phrase, pronunciation focus, "
    "or a follow-up question. Keep responses to 1-4 short sentences."
)
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=os.environ.get("SPEAK_ALLOWED_ORIGINS", "http://localhost:3000").split(","),
...
@app.get("/")
async def root():
    return {"service": "csit998-speak", "websocket": "/ws", "model": HF_REPO}
